...
class And < Not
  def match(text)
    @child.match(text) ? result('') : nil
  end
end

#class Optional < Not
#  def match(text)
#    @child.match(text) || result('')
#  end
#end
...
class Grammar
  ...
  def self._peg
    ...
    literal = Sequence.new(Regex.new("'.*?'"), spacing).name('literal')
    ...
    expression = Sequence.new(
                   sequence,
                   ZeroOrMore.new(
                     Sequence.new(slash, sequence).name('expression__sequence')
